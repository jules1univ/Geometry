# Liste des membres

| Nom        | Prénom | Github      | Mail                                      | Rôle          |
| ---------- | ------ | ----------- | ----------------------------------------- | ------------- |
| GARCIA     | Jules  | jules1univ  | jules.garcia.1@etudiant.univ-rennes1.fr   | Scrum Master  |
| EYLEM      | Kerem  | Keylem      | kerem.eylem@etudiant.univ-rennes1.fr      | Product Owner |
| BOITEL     | Briac  | bboitel     | briac.boitel@etudiant.univ-rennes.fr      | Developer     |
| BERTHELIER | Noé    | nberthelier | noe.berthelier@etudiant.univ-rennes1.fr   | Developer     |
| BARBIER    | Elouan | Marsu2      | elouan.barbier.1@etudiant.univ-rennes1.fr | Developer     |
| GALLON     | Paul   | MarcoPaulot | paul.gallon@etudiant.univ-rennes.fr       | Developer     |
| GUEMENE    | Basile | Astala-Boom | basile.guemene@etudiant.univ-rennes1.fr   | Developer     |

# Communication

La communication du projet s'effectue principalement via [le chat du repo](https://talktogithub.com/jules1univ/Geometry).
